...
import { ProblemChatComponent } from "./problem-chat.component";
...
export class ProblemsComponent implements OnInit {
  ...
  get filteredProblems(): Problem[] {
    return this.problems.filter((problem) => {
      ...
      if (
        ...
        problem.severity !== this.filterSeverity
      ) {
        return false;
      }
      if (
        this.filterCategory !== "all" &&
        problem.category !== this.filterCategory
      ) {
        return false;
      }
      return true;
    ...
    this.apiService
      ...
      .subscribe({
        ...
        error: (error) => {
          ...
          alert(
            "Erreur lors de la création de la résolution. Vérifiez la console."
          );
        },
      });
  }
  ...
  markAsResolved(resolution: ProblemResolution) {
    ...
    if (confirm("Confirmer que cette solution a résolu votre problème ?")) {
      this.apiService
        .markResolutionSuccessful(resolution.id, userId)
        .subscribe({
          next: (data: any) => {
            ...
            if (
              ...
              this.selectedProblem.id === data.problem_id
            ) {
  ...
  closeProblem() {
    ...
    if (
      confirm(
        "Fermer définitivement ce problème ? Aucune nouvelle solution ne pourra être ajoutée."
      )
    ) {
  ...
  reopenProblem() {
    ...
    if (
      confirm(
        'Rouvrir ce problème ? Il repassera en statut "Ouvert" et d\'autres solutions pourront être ajoutées.'
      )
    ) {
  ...
  adminSoftDeleteProblem(problem: Problem) {
    ...
    if (
      ...
      this.apiService
        .adminSoftDeleteProblem(problem.id, this.currentUserId)
        .subscribe({
          next: (response) => {
  ...
  loadDeletedProblems() {
    ...
    this.apiService.adminGetDeletedProblems(this.currentUserId).subscribe({
      ...
      error: (error) => {
        console.error(
          "Erreur lors du chargement des problèmes supprimés:",
          error
        );
  ...
  adminRestoreProblem(problem: Problem) {
    ...
    if (confirm(`Restaurer le problème "${problem.title}" ?`)) {
      this.apiService
        .adminRestoreProblem(problem.id, this.currentUserId)
        .subscribe({
          next: (response) => {
            // Retirer de la liste des supprimés
            this.deletedProblems = this.deletedProblems.filter(
              (p) => p.id !== problem.id
            );
  ...
  adminSoftDeleteResolution(resolution: ProblemResolution) {
    ...
    if (
      !confirm(
        `Êtes-vous sûr de vouloir supprimer cette résolution ? Cette action peut être annulée.`
      )
    ) {
      return;
    }
    ...
    this.apiService
      .adminSoftDeleteResolution(resolution.id, this.currentUserId)
      .subscribe({
        next: () => {
  ...
  loadDeletedResolutions() {
    ...
    this.apiService.adminGetDeletedResolutions(this.currentUserId).subscribe({
      ...
      error: (error) => {
        console.error(
          "Erreur lors du chargement des résolutions supprimées:",
          error
        );
      },
    });
  ...
  adminRestoreResolution(resolution: ProblemResolution) {
    ...
    this.apiService
      .adminRestoreResolution(resolution.id, this.currentUserId)
      .subscribe({
        next: () => {
          // Retirer de la liste des supprimés
          this.deletedResolutions = this.deletedResolutions.filter(
          ...
          if (
            this.selectedProblem &&
            this.selectedProblem.id === resolution.problem_id
          ) {
            this.loadResolutions(this.selectedProblem.id);
          }
